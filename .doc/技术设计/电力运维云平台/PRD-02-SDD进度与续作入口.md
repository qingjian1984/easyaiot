...
> 最近归档：本轮 G2-03 `APPROVE_NONE_ENABLED` 证据签署（DEVICE_PROTOCOL_OWNER_EVIDENCE.json + 确认单 0.3.0）
...
2026-08-26 初始仓库证据复核形成 C1P-G2 输入单 0.1.0；当前已演进为 [0.8.0](./P02-M2-02C1P-G2-责任人输入收集与建议处置单.md)。初始检查点 `528e93736` 不包含四类责任人签署；其后只有 G2-01 获得部分可接受签署。输入包只把仓库事实、推荐选择、必交证据和签署顺序固化，不把推荐值、合同或验收工具准备视为批准，不解锁 C1A。

G2-01 产品签署已在提交 `d7d77d5c6` 归档。Sol 复核后接受 severity、范围、disabled、隔离和 activation 语义，但 activationAt 仍为 `PENDING-PREVIEW`，且签署对象把实际输入单 `0.1.0` 写成 `1.0.0`，因此状态为 `PARTIAL-ACCEPTED / SIGNED-PENDING-ACTIVATION`，而非完全关闭。提交 `1cb25b730` 已归档 [G2 输入处置记录](./P02-M2-02C1P-G2-输入处置记录.md)，提交 `d37054a41`、`20c226ec1`、`94b2cb79e`、`9478fef98` 已依次归档 G2-03/G2-04 Schema、模板和本地验收器，提交 `2a22b22a4` 已归档 G2-02 Owner evidence Schema、DRAFT 模板与确认单 0.2.0。其后 C1P-G2-02 verifier 归档于 `c598ceafc`、G2-02 确认单升级 0.3.0。既有 2026-08-12 TD-003 设计签署仍不足以确认 G2-02 的发布历史、v1/v2、生产 Schema 和测试计划。

2026-08-27 G2-03 设备协议 owner 以 `APPROVE_NONE_ENABLED` 签署本轮盘点：[OWNER_EVIDENCE](./assets/c1p-g2/DEVICE_PROTOCOL_OWNER_EVIDENCE.json) 覆盖全部 8 类已盘点协议（6 类 DISABLED、SUB_EVENT UNSUPPORTED、采集器 OUT_OF_SCOPE），本地验收 `PASS protocols=8 enabled=0 qualification=NO_ENABLED_PROTOCOL`，[G2-03 确认单升级 0.3.0](./P02-M2-02C1P-G2-03-设备事件协议证据与Owner确认单.md)。该签署关闭 G2-03 本轮盘点责任，但 G2-03 功能门禁（至少一个 direct 协议 ENABLED）保持 OPEN，真实 mapping 仍为 0，`DENY_ALL` 与 C1A 关闭状态不变；启用任一协议需先另行授权 codec/transport 整改（保留 wire 时间、commit 后 ACK、retry/collision fixture）。
...
| C1P-G2 | [输入单 0.8.0](./P02-M2-02C1P-G2-责任人输入收集与建议处置单.md)、[处置记录 0.7.0](./P02-M2-02C1P-G2-输入处置记录.md)、[G2-02 Owner 确认单 0.3.0](./P02-M2-02C1P-G2-02-投影事件Owner确认单.md)、[G2-03 Owner 确认单 0.3.0](./P02-M2-02C1P-G2-03-设备事件协议证据与Owner确认单.md)、[G2-04 Owner 确认单](./P02-M2-02C1P-G2-04-设备事件产品Allowlist与Owner确认单.md) | G2-01 部分接受、待 activation/勘误；G2-03 盘点已签署 APPROVE_NONE_ENABLED（功能门禁仍 OPEN）；G2-02/G2-04 仍待各 Owner，DEVICE_EVENT 全部协议关闭、DENY_ALL；C1 关闭 |
...
| [P02-M2-02C1P-G2](./P02-M2-02C1P-G2-责任人输入收集与建议处置单.md) | 四类责任人输入的仓库事实、推荐选择、签署字段和关闭顺序 | **G2-01 Partial-Accepted / G2-03 Inventory-Signed-APPROVE_NONE_ENABLED（功能门禁 OPEN）/ G2-02/G2-04 Local-Verifier-Prepared / G2-04 Deny-All / C1 Closed**；不得把验收工具或部分签署解释为实现授权 |
...
1. ~~设备协议 owner 提交 G2-03 证据~~（2026-08-27 已以 `APPROVE_NONE_ENABLED` 完成本轮盘点签署，8 类协议全部如实 DISABLED/UNSUPPORTED/OUT_OF_SCOPE，`qualification=NO_ENABLED_PROTOCOL`）。G2-03 功能门禁仍 OPEN：后续若要启用协议，需决策所有者先授权 codec/transport 整改切片（保留 wire occurredAt、commit 后 ACK、retry/collision fixture、禁止后端补 ID），整改完成后再由 owner 提交 revision 2 证据并标记 ENABLED。
2. 至少一个 G2-03 direct 协议通过并标记 ENABLED 后，产品 owner 再按 [G2-04 Owner 确认单](./P02-M2-02C1P-G2-04-设备事件产品Allowlist与Owner确认单.md)提交 mapping、五类必备 fixture、条件 PREDICATE_TYPE_MISMATCH、COMPLETE review package、APPROVED allowlist 和签署，并运行 `pnpm verify:device-event-alarm-allowlist-review -- --file .doc/.../OWNER_REVIEW.json`；此前保持 `DENY_ALL`。
3. M1/TD-003 owner 从 [G2-02 DRAFT 模板](./assets/c1p-g2/telemetry-projected-owner-evidence-template.json)复制独立 evidence，明确发布历史、v1/v2、更新 TD、生产版本化 Schema/API、测试计划及条件性消费者清单，运行 `pnpm verify:telemetry-projected-owner-evidence -- --file .doc/.../OWNER_EVIDENCE.json` 通过后按 [G2-02 确认单](./P02-M2-02C1P-G2-02-投影事件Owner确认单.md)签署；产品规则 owner 同时提交 G2-01 输入单版本勘误。activationAt 只能在迁移预览清单冻结后补签，不得预填；当前不得创建 C1A。
4. P02-M2-02A 临时 PostgreSQL 合同保持 `NOT RUN`；只有决策所有者另行明确授权新建、唯一前缀、用后销毁的隔离实例后才可执行，目标/共享库始终不在授权内。
5. 临时库授权前不得读取 `.env` 凭据、调用 Docker、连接既有 PostgreSQL 或执行 V011/U011；02B JDBC 真实并发/约束集成继续为 OPEN。
6. ADR-019 Proposed 不解锁 C1；四项 C1 门禁全部达到 `CLOSED-CONTRACT`、ADR 经决策所有者 Accepted 且同提交更新索引前，C1-C4 不得实现。
7. TD-008、外部通知和完整事故冻结继续关闭。
