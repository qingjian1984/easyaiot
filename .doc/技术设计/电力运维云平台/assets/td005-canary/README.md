...
无活动访问令牌；full 档位 WEB 已首次部署并 healthy，8888 登录面就绪。用户正常登录和独立 Canary
写入批准两项未关闭前，不得进入 Canary 写请求。

浏览器认证窗口已获批准，但应用内浏览器控制在导航前因主机权限无法建立连接；页面和认证 API 均未访问，
token 仍为 0。须修复应用内连接或另批 Chrome CDP 控制面后重试。
