...
- **ADR-015（Proposed）已形成并提交（2026-08-10，commit `db2e9bd6`）**：处置 V003 第 14-16 行登记的 OPEN 项「活动 collector workload binding 的设备侧可见性」。决策（方案 E）：iot-device 库新建独立可变投影表 `collector_workload_binding_projection`，由 iot-device 自身发布单状态机同事务 upsert（人工首次发布 + 协调器再生 + workload 停止三条路径），不依赖 iot-node 事件同步——workload 存在性由「有发布单」隐含。研究依据：iot-device 与 iot-node 完全解耦（无 pom/Feign/事件/共享库，不同库），`node_workload_binding` 无 `tenant_id/product_id/device_id/site_id`，方案 A（直连）/B（RPC）技术不可行且违宪（§5.2 L224 禁跨库查、§6.2 L264 禁跨服务共享表）。经 architect 对抗性审查否决方案 D（扩展 `iot_collector_config_release` 加来源字段）：首次发布死循环（CRITICAL，人工 API 不经 `createRegenerationDraft` 致来源字段缺失，空集伪装无受影响，违反 §6.2 不得静默吞错）、职责混淆（HIGH）、触发器保护范围冲突（MEDIUM）。V003 第 14-18 行 OPEN 项标注已更新指向 ADR-015。投影表 DDL 候选见 ADR-015，待评审通过后增链 V003 或独立 V004，落库 MUST 经 ADR-013 runner。状态 Proposed，无代码改动。
...
0. **ADR-013 1.5.2/ADR-014 1.3.6 已 Accepted，V001 落库窗口已执行（2026-08-08）**：三项人工闭环关闭 + 五步骤链对 iot-device20 apply SUCCEEDED；TD-005 冻结六批实现证据落地 + T-18 批次 1（协调器四处理器，197/197 PASS）+ 批次 2 第一步（V003 三表 DDL 资产 + runner 增链，烟测 PASS）；**当前最优先：ADR-015（Proposed）评审 → 通过后增链投影表 DDL 落库 → `CollectorWorkloadImpactPort` JDBC 实现；T-18 批次 2 后续（V003 新窗口批准落库 → 三端口 JDBC 实现 + 真实库合同测试），以及 manifest 真实 Git commit/hash 写入后 TD-005 Approved/Frozen 决策**；`power_model_event_inbox` 需 runner 增链步骤落库（新窗口）；
