...
0. **当前最优先：ADR-013/014 转 Accepted 的收尾**。可本机完成的证据已全部 PASS（评审报告 §8.3/§8.4、ADR-013 1.4.3 证据表）；① DBA/代码 owner 复核签字已双签关闭（签字包 §5/§5.1）；② standard 最低规格压测经 owner 决定豁免（候选预算保持未冻结）；**仅余 ③ 完整 12 表画像生产重跑**（按[Runbook](./TD-005-生产画像重跑Runbook.md)，需生产只读窗口与审批）。③ 关闭后 ADR-013/014 可转 Accepted；
