...
0. **ADR-013 1.5.2/ADR-014 1.3.6 已 Accepted，V001 落库窗口已执行（2026-08-08）**：三项人工闭环关闭 + 五步骤链对 iot-device20 apply SUCCEEDED；TD-005 冻结六批实现证据落地 + T-18 批次 1（协调器四处理器，197/197 PASS）+ 批次 2 第一步（V003 三表 DDL 资产 + runner 增链，烟测 PASS）；**当前最优先：ADR-015（Proposed）评审 → Accepted → 投影表 DDL 增链落库（V003 扩展第四张表或独立 V004，新窗口）→ 三已实现端口（Audit/TemplateReference/Impact）PG 真实库合同测试；`CollectorConfigReleasePort` 阻塞于 ConfigSnapshot 组装（类型待建）+ 任务 7 发布管线 upsert 写入端，待任务 7 推进；V003 新窗口批准落库后补真实库测试；manifest 真实 Git commit/hash 写入后 TD-005 Approved/Frozen 决策**；`power_model_event_inbox` 需 runner 增链步骤落库（新窗口）；
