...
| [TD-005 物模型模板 Schema、版本差异与发布 API](./TD-005-物模型模板Schema版本差异与发布API.md) | 1.0.13 | In Review |
| [TD-005 运行模型兼容与删除链技术设计](./TD-005-运行模型兼容与删除链技术设计.md) | 0.1.6 | In Review |
...
- 本地目标集成实例 12 表画像、R1～R7 文档处置、4条孤儿属性清理、ADR-012 接受、迁移前非空 golden、Mapper/DO/根属性 DTO/legacy 只读 adapter、Java 双向纯转换 golden 合同及根属性 TEN-001～004/006 已完成；仍需完成八表数据库持久化接线、其余 TEN、唯一约束和删除链合同；生产存量环境需按画像 Schema 重跑；
...
1. 读取 [TD-005 1.0.13](./TD-005-物模型模板Schema版本差异与发布API.md)、[TD-005 运行模型兼容与删除链设计 0.1.6](./TD-005-运行模型兼容与删除链技术设计.md)和[TD-005 评审报告 §22](../../开发规范/TD-005评审报告.md)；
2. Mapper/DO/VO、legacy 只读 adapter、Java 双向纯转换 golden 与根属性 PostgreSQL TEN-001～004/006 已完成，定向测试累计 13 项 PASS；先复跑该测试集，禁止回退到 `product_properties.service_id`；
3. 下一步把 `LegacyThingModelRuntimeAdapter` 接入 tenant-safe 八表持久化与导出服务，补跨服务/命令/参数 TEN-005、稳定错误和事务回滚测试；证据通过后形成数据库 migration/rollback 候选，但暂不启用约束；
...
> 读取 `.doc/技术设计/电力运维云平台/M1-SDD进度与续作入口.md`，遵循《平台功能计划》和《EasyAIoT 项目开发宪法》，继续 TD-005。TD-005 1.0.13 / 运行模型 0.1.6 已完成 12 表目标画像、结果 Schema 1.1.0、迁移前非空旧格式 golden、Mapper/DO/根属性 DTO/legacy 只读 adapter、主代码 Java legacy 双向纯转换，以及真实 PostgreSQL 根属性 TEN-001～004/006；Java 17 编译和 13 项合同测试 PASS，测试事务回滚后残留为 0。ADR-012 1.0.2 已 Accepted，整体仍为 OPEN_REMEDIATION_REQUIRED。下一步把转换 adapter 接入 tenant-safe 八表持久化与导出服务，补 TEN-005、稳定错误与事务回滚测试，再形成 migration/rollback 候选。
