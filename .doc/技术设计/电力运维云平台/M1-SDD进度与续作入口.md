...
0. **ADR-013 1.5.2/ADR-014 1.3.6 已 Accepted，V001 落库窗口已执行（2026-08-08）**：三项人工闭环关闭 + 五步骤链对 iot-device20 apply SUCCEEDED；TD-005 冻结六批实现证据落地 + T-18 批次 1（协调器四处理器，197/197 PASS）+ 批次 2 第一步（V003 三表 DDL 资产 + runner 增链，烟测 PASS）；**当前最优先：T-18 批次 2 后续（V003 新窗口批准落库 → 三端口 JDBC 实现 + 真实库合同测试），以及 manifest 真实 Git commit/hash 写入后 TD-005 Approved/Frozen 决策**；`power_model_event_inbox` 需 runner 增链步骤落库（新窗口）；
