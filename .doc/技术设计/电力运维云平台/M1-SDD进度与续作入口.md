...
| [TD-001 collector 与 NODE 部署契约](./TD-001-collector与NODE部署契约.md) | 1.0.7 | In Review |
...
- T-18 批次 2 第二步（JDBC 端口实现 2/4）已落地并提交（2026-08-08，commit `82c88680`，TD-001 1.0.7 仍 In Review、ADR-013 1.5.2 / ADR-014 1.3.6 不变）：实现四协调端口中的两个 JDBC 实现——`JdbcPowerModelCoordinationAuditPort`（`power_model_coordination_audit` 追加写，触发器拒绝 UPDATE/DELETE，`(tenant_id,event_id,action)` ON CONFLICT DO NOTHING 重投重试不覆盖不膨胀，detail 防御性截断 ≤512）与 `JdbcPowerModelTemplateReferencePort`（`power_model_template_reference_mark` 同 `(tenant_id,template_code,template_version,to_lifecycle)` upsert 刷新标记时间与来源事件）；并对齐接口签名与 V003 DDL 列契约：`record()` 增 `eventType`（`event_type` 列）、`markLifecycleReference()` 增 `sourceEventId`（`source_event_id` 列，幂等溯源），处理器 4 处 `auditPort.record` 调用同步 `envelope.eventType()`，测试 FakeAudit 同步并增 eventType 透传断言；ID 由应用统一雪花策略赋值，不创建任何数据库对象。**本次同时修复了该批次的编译失败中间状态**（接口加 eventType 但调用方/测试 fake 未同步）。验证：`PowerModelCollectorEventHandlersTest` 12/12 PASS，iot-device-biz 全量回归 197/197 PASS（21 项设计内 PG 跳过，与基线一致）。OPEN：`CollectorWorkloadImpactPort`（只读既有表，无新 DDL）与 `CollectorConfigReleasePort`（写 `iot_collector_config_release`，含 canonical/hash/状态机）两端口 JDBC 实现待续；两已实现端口的真实库合同测试待 V003 新窗口批准落库；`PowerModelEventWiringConfiguration` 四端口齐备前注册表仍空（事件按「处理器缺失 → DLQ」处置，有持久证据非静默丢弃）。
...
0. **ADR-013 1.5.2/ADR-014 1.3.6 已 Accepted，V001 落库窗口已执行（2026-08-08）**：三项人工闭环关闭 + 五步骤链对 iot-device20 apply SUCCEEDED；TD-005 冻结六批实现证据落地 + T-18 批次 1（协调器四处理器，197/197 PASS）+ 批次 2 第一步（V003 三表 DDL 资产 + runner 增链，烟测 PASS）；**当前最优先：T-18 批次 2 JDBC 端口剩余 2/4（`CollectorWorkloadImpactPort` 只读既有表 → `CollectorConfigReleasePort` 写 `iot_collector_config_release` 发布单，含 canonical/hash/状态机），V003 新窗口批准落库后补两已实现端口的真实库合同测试，以及 manifest 真实 Git commit/hash 写入后 TD-005 Approved/Frozen 决策**；`power_model_event_inbox` 需 runner 增链步骤落库（新窗口）；
