...
> 检查点日期：2026-08-24
> Git 分支：`cfdqiot`（全部推送 origin，最新 `9f3067dd`：记录 MQTT→Inbox 全链路与装配缺陷族）
> 当前阶段：OPEN03 门禁 1/2/3/6 已由 Sol 关闭为 CLOSED-LOCAL；LC02A-0～4 Verified-Local；M1-LC-02 已 Approved / Frozen；LC02-01A / R1 / 04A / 04B / 04C-1 / 04C-2 / 05 / 06 / 07 / 08 / 09-R1 已 COMPLETE / SOL-ACCEPTED；LC02-10 因 sink 定向回归 3 Errors 保持 STOPPED / NOT ACCEPTED；LC02-10-R1 §25.2 已由 Sol 接管实现并完成 Linux 直接测试 5/5，完整验收因两组 PG 凭据未注入而 NOT RUN；V009 正式落库与生产 MQTT 凭据/ACL 激活仍 OPEN / NOT APPROVED；TQ 查询/展示面已部署上线，MQTT→Inbox 真实容器链路已打通（见 2026-08-24 记录）
...
- **当前下一步**：在实际调用进程显式提供非空 `LC02_V009_PG_PASSWORD`、`LC02_08_PG_PASSWORD`，随后由 owner 独立授权执行 LC02-10-R1 §25.3～§25.4 完整验收；在此之前不得复用 5/5 直接测试关闭 LC02-10。正式 V009 落库仍须另行批准且部署顺序必须是“V009 目标迁移/核验 → LC02-08 Java 部署 → 中心入口启用验证”。`LC02-09-RUNTIME-01`、生产凭据/ACL/TLS 激活、Linux PTY/锁互操作、资源压测、Windows 发布资格和现场证据继续锁定/OPEN。
