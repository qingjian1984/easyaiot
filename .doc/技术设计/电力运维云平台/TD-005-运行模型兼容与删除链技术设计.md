...
> 版本：0.1.7
> 日期：2026-08-05
> 上游：[ADR-012 1.0.2 产品根属性与服务参数单一事实](../../架构决策/电力运维云平台/ADR-012-产品根属性与服务参数单一事实.md)、[TD-005 1.0.14](./TD-005-物模型模板Schema版本差异与发布API.md)
...
该证据只把**根属性层的 TEN-001～004/006 子合同**转为 PASS，不代表 TEN 套件整体通过。TEN-004 的稳定业务错误映射、跨表 TEN-005、standard/full TEN-007、mini capability TEN-008，以及八表 importer/exporter 的 tenant-safe 持久化、事务回滚和接口接线仍为 OPEN；不得据此启用 DDL 约束或替换旧写入口。
...
本服务尚未接入公开 `/thingModel` 写/读端点，也未完成稳定业务错误映射、审计/Outbox、TEN-007、TEN-008、DDL unique/XOR/tenant FK/RESTRICT、DEL、TypeScript、三档回归和性能证据。旧公开发布仍是无持久化占位，禁止把内部服务完成描述为 TD-005 已上线或已冻结。
