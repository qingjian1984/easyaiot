...
> 版本：0.1.4
> 日期：2026-08-05
> 上游：[ADR-012 1.0.2 产品根属性与服务参数单一事实](../../架构决策/电力运维云平台/ADR-012-产品根属性与服务参数单一事实.md)、[TD-005 1.0.11](./TD-005-物模型模板Schema版本差异与发布API.md)
...
参考验证已对运行投影、旧格式输出、JCS canonical 和 manifest SHA-256 执行 PASS，canonical SHA-256 为 `b0457e19d65a5de810222ab2ee5ac92fdafcb799058d3857944f88aa66e70262`。本证据完成了 §9/§12 要求的 Mapper/DO/VO 迁移前置冻结，因此下一步可开始生产实现；但只有 Java importer/exporter、TypeScript 客户端与 PostgreSQL tenant 集成测试消费同一 golden 后，兼容实现子门禁才能转 PASS。
...
本批次只把“确定性 Mapper/DO/VO 漂移”和“旧 service-property 只读投影”从未实现推进为已有单元/合同证据，**尚未关闭整个兼容实现门禁**。以下事项仍为 OPEN：冻结 golden 的生产 Java importer/exporter 全量消费、PostgreSQL tenant CRUD/拦截器集成、命令影子 `service_id` 一致性校验、旧缓存与 Feign 消费者合同、migration/rollback、唯一/XOR/FK/RESTRICT、TEN/DEL、三档回归和性能证据。不得启用 DDL 约束，也不得进入产品删除链上线。
