...
> 版本：0.1.9
> 日期：2026-08-06
> 上游：[ADR-012 1.0.2 产品根属性与服务参数单一事实](../../架构决策/电力运维云平台/ADR-012-产品根属性与服务参数单一事实.md)、[TD-005 1.0.16](./TD-005-物模型模板Schema版本差异与发布API.md)
...
| 0.1.6 | 在真实 PostgreSQL 事务中验证根属性 TEN-001～004/006 子合同，修复混租户批量删除部分成功风险；定向测试累计 13 项 PASS |
| 0.1.7 | 新增 tenant-safe 八表聚合持久化边界（锁定产品、事务替换七张子表），完成 TEN-005、导出和数据库失败回滚证据；定向测试累计 17 项 PASS |
| 0.1.8 | 接入 ADR-011 统一 capability 与稳定业务错误，完成 standard/full TEN-007、mini TEN-008 和只读能力 API；审计/Outbox 仍待批准迁移 |
| 0.1.9 | 承接 migration 子设计 0.1.1：冻结幂等表 M0.5、product 同租户 unique M1.5、binding FK 和审计/Outbox 原子前置；运行表约束与删除链仍不得提前启用 |
...
参考验证已对运行投影、旧格式输出、JCS canonical 和 manifest SHA-256 执行 PASS，canonical SHA-256 为 `b0457e19d65a5de810222ab2ee5ac92fdafcb799058d3857944f88aa66e70262`。本证据完成了 §9/§12 要求的 Mapper/DO/VO 迁移前置冻结；主代码 Java 纯转换 adapter 已在 §14.5 消费同一 fixture/golden，PostgreSQL 八表聚合持久化子合同已由 §14.6～§14.7 补齐。TypeScript 客户端、公开接口接线、其余 TEN/DEL 和数据库约束仍未完成，因此兼容实现总门禁仍不能转 PASS。
...
### 14.6 PostgreSQL 根属性租户 CRUD 证据（2026-08-05）

新增 opt-in 的 `ProductPropertiesTenantPostgresIntegrationTest`，连接本地 `postgres-server / iot-device20`，每个测试使用唯一 fixture、关闭自动提交并在结束时强制回滚，不修改约束或既有数据。测试后以只读 SQL 确认测试 tenant/property 前缀残留行为 0。

本轮覆盖并通过以下根属性子合同：

- TEN-001：DO 即使携带伪造 `tenantId`，INSERT 列仍由 `TenantLineInnerInterceptor` 从服务端上下文注入，数据库实际值等于当前 tenant；
- TEN-002：tenant 2 使用 tenant 1 的主键查询返回空；
- TEN-003：tenant 2 更新或删除 tenant 1 的行均影响 0 行，tenant 1 原数据保持不变；
- TEN-004：批量 ID 混入其他 tenant 时影响 0 行，两侧数据均保留；
- TEN-006：缺少 tenant context 时由 `TenantContextHolder.getRequiredTenantId()` 在执行 INSERT 前 fail-closed。

首次运行暴露出 `TenantLineInnerInterceptor` 只保护 DELETE 外层、未自动限制完整性计数子查询，混租户批量删除会部分成功。`deleteProductPropertiesByIds` 已将计数子查询显式关联到外层 `product_properties.tenant_id`，并用绑定参数比较期望行数；真实 PostgreSQL 复跑 PASS，静态 Mapper 合同同时禁止 `${...}` 文本替换。

联合运行 `ProductPropertiesMapperContractTest`、`LegacyServicePropertyAdapterTest`、`LegacyThingModelRuntimeAdapterGoldenTest` 和 PostgreSQL 集成测试：13 tests，0 failure，0 error，0 skipped，Java 17 反应堆 `BUILD SUCCESS`。

该证据只把**根属性层的 TEN-001～004/006 子合同**转为 PASS；跨表 TEN-005、稳定业务错误、standard/full TEN-007 和 mini TEN-008 的后续执行证据见 §14.7～14.8。公开接口、DDL 与删除链门禁仍未关闭，不得据此启用约束或替换旧写入口。

### 14.7 八表聚合持久化、TEN-005 与回滚证据（2026-08-05）

新增内部 `LegacyThingModelPersistenceService`，作为 `LegacyThingModelRuntimeAdapter` 与数据库之间的唯一候选持久化边界。八表运行投影中的 `product` 仅作为聚合根身份：服务按“当前 tenant + productIdentification”锁定且要求唯一的既有产品，不创建或覆盖产品档案；随后在同一 `@Transactional(rollbackFor = Exception.class)` 事务中按依赖顺序替换另外七张模型表。导出使用 `@Transactional(readOnly = true)`，从同租户关联链重建运行投影，再交由 adapter 恢复旧格式。

实现的安全边界：

- runtime product、根属性、服务、命令、输入/输出参数、事件和事件输出的 `tenantId` 必须全部等于 `TenantContextHolder.getRequiredTenantId()`；
- 根属性、服务和事件必须属于同一 product scope，禁止混入 template scope；
- command 的 `serviceId` 必须引用本次同租户 service；command request/response 的 `commandsId` 与影子 `serviceId` 必须同时匹配该 command；event response 必须引用本次 event 且不得携带 `serviceId`；
- 所有查询、锁、删除、插入和关联 JOIN 均显式绑定 tenant；动态命令参数表名只有 request/response 两个内部白名单值；
- 删除顺序为 event response→event、command request/response→command→service、root property，插入顺序反向满足父子依赖；数据库异常由外层事务整体回滚。

`LegacyThingModelPersistencePostgresIntegrationTest` 在真实 `postgres-server / iot-device20` 上完成 4 项证据：事务注解合同；既有产品锁定后七张子表写入及旧格式导出；TEN-005 的跨租户参数与 service-command 影子关系不一致均在删除前拒绝；删除并部分插入后触发 NOT NULL 异常，回滚到事务保存点后原七表模型完整恢复。测试使用唯一 product fixture，外层连接最终整体回滚；八张表测试 tenant 残留总数为 0。

联合既有 Mapper、根属性 PostgreSQL、legacy adapter 和 frozen golden 测试共 17 tests，0 failure，0 error，0 skipped，Java 17 反应堆 `BUILD SUCCESS`。因此 **Java 内部八表聚合持久化/导出、TEN-005 应用校验和数据库失败回滚子门禁 PASS**。

本服务尚未接入公开 `/thingModel` 写/读端点，也未完成审计/Outbox、DDL unique/XOR/tenant FK/RESTRICT、DEL、TypeScript、三档端到端回归和性能证据。稳定业务错误及 TEN-007/008 已由 §14.8 补齐；旧公开发布仍是无持久化占位，禁止把内部服务完成描述为 TD-005 已上线或已冻结。

### 14.8 稳定错误与档位合同证据（2026-08-05）

ADR-011 规定目录现已包含 Draft 2020-12 schema 及 standard/full 两份版本化 manifest。共享 `CapabilityService` 位于 `iot-common-env`，业务代码只查询 `power.device.model`，没有新增 standard/full 两套 Controller、Service、表或 SQL。`iot-system` 暴露只读 `/system/capabilities`；安装器和 Docker Compose 传递同一 profile、文件与内容，未配置时所有电力能力 fail-closed。standard 旧部署裁剪同步修正为包含 `iot-device`。

内部持久化边界新增前置 capability guard 和 `1_003_023_000～010` 稳定错误映射。TEN-007 在同一 PostgreSQL fixture 上依次用 standard/full manifest 执行同一替换流程，剔除每次合法重建的代理 ID 后，legacy 业务语义、表行数和错误合同一致；TEN-008 用 mini fail-closed snapshot 调用无效输入，先返回 `CAPABILITY_NOT_SUPPORTED`，模型表无读写副作用。

共享 capability 合同 4 tests、只读 API 1 test 与设备侧目标集合 19 tests 均为 0 failure、0 error、0 skipped，Java 17 reactor `BUILD SUCCESS`；测试事务结束后两个 tenant 在 product 与七张子表的残留总数为 0。TEN-007/008 和当前内部边界稳定错误子门禁转为 PASS。审计/Outbox 需要先批准版本/绑定/outbox migration 才能与业务事务原子验证，本批次保持 OPEN，不创建无设计依据的临时表。

### 14.9 版本层 migration 前置对齐（2026-08-06）

[版本/绑定/审计/Outbox migration 子设计 0.1.1](./TD-005-版本绑定审计Outbox迁移与回滚设计.md)明确 binding 以 `(tenant_id, product_identification)` 复合 FK 指向 product，`product_id` 作为同一锁定行的不可变审计副本并由 trigger 校验。目标画像当前没有该业务 unique，因此执行顺序必须为：生产画像重复为 0 → `CREATE UNIQUE INDEX CONCURRENTLY` → 附加稳定 UNIQUE 约束 → 创建 binding FK `NOT VALID` → 独立 VALIDATE。

版本写 API 还依赖 TD-004 `power_idempotency_record` 在 M1 前落库并通过跨副本/恢复合同。两项前置和 migration runner 未批准前，现有八表内部持久化不得接公开版本/绑定接口，也不得启用 unique/XOR/FK/RESTRICT 或删除链。该同步仅关闭文档顺序歧义，不构成 DDL、DEL 或生产迁移证据。
