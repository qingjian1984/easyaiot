...
> 状态：**Accepted**（2026-08-08：三项人工闭环全数关闭——① DBA/代码 owner 双签 2026-08-07；② standard 最低规格压测 owner 豁免 2026-08-07；③ 完整 12 表画像生产重跑 2026-08-07 执行 PASS、2026-08-08 owner 指定满足 productionRerunRequired）
> 版本：1.5.7
> 日期：2026-08-07
...
| 1.5.0 | 2026-08-08 | **转 Accepted**：owner（青见/qingjian1984）审阅证据包后指定 2026-08-07 画像重跑满足 `productionRerunRequired=true`（指定记录见证据包 verdict.md「owner 指定记录」）；三项人工闭环（双签 / 压测豁免 / 画像生产重跑）全数关闭，本 ADR 与 ADR-014 转 Accepted。后续冻结工作（事件消费者/transport/Inbox 实现、CI 双主版本门禁接线）按 TD-005 计划推进；候选预算值保持未冻结，生产执行前按当时环境人工评估 |
| 1.5.1 | 2026-08-08 | **V001 落库窗口已批准并执行**：owner 批准窗口（申请单 `assets/td005-migration/V001落库窗口申请单-20260808.md`），runner 对 iot-device20 执行五步骤链 **apply SUCCEEDED**（APPROVAL=V001-WINDOW-20260808，执行前自动 pg_dump 备份于仓库外）；执行后只读验证 history 五行 SUCCEEDED、invalid_indexes=0、MIG-009 注释门禁 PASS、业务计数不变（product=4/device=4/product_properties=17）、Outbox 空表。窗口镜像演练证据（schema-only 镜像库全链 SUCCEEDED 后删库）与执行记录归档于 `assets/td005-migration/verification/window-rehearsal-20260808/` 与 `window-execution-20260808/`。如实记录 runner 观察：脚本头注释称 PG_CONTAINER 默认 postgres-server 但实现无默认赋值，不显式传参会走本地 psql 分支（本执行已显式传参规避，修正列后续小版本）。V001/V002 表结构自此在目标实例就位，Outbox/Inbox 真实库合同（fcc937c8）与目标实例结构一致 |
| 1.5.2 | 2026-08-08 | **runner 增链 V003（TD-001 协调器三表，评审候选，未执行）**：新增资产 `V003__iot_collector_coordination.sql`（iot_collector_config_release 发布单[TD-001 §4.1] + power_model_template_reference_mark 引用标记 + power_model_coordination_audit 追加写审计；快照不可变触发器、canonical 长度/hash CHECK、审计追加写触发器）与 `U002__iot_collector_coordination.sql` 卸载候选；runner APPLY_STEPS 扩展为 M05→M15→M16→V001→V002→V003（事务型，CLI/env/注释门禁清单同步）。临时评审库烟测全项 PASS（建表/唯一/hash/canonical 长度 CHECK/快照不可变/审计追加写/9 反例/引用标记幂等 upsert/U002 非空拒绝，MIG-009 PASS，库用后已 DROP）。如实 OPEN：① V003 对目标实例落库须新窗口申请与批准，本版本不构成执行授权；② U002 卸载链尚未接入 runner uninstall 驱动（当前仅 U001），接入前回滚路径为应用层停用 + 人工候选脚本；③ CollectorWorkloadImpactPort 只读既有表无需新 DDL，「活动 collector workload binding」设备侧可见性为 TD-001 设计 OPEN |
| 1.5.3 | 2026-08-10 | 登记既有 V004/V005 目标窗口执行事实，并将 TD-004 核心关系 V006 接入 apply 链：新增五表 V006、空表拒绝 U005，CLI/env/MIG-009 清单同步。临时库 runner 首次 SUCCEEDED、重放 SKIPPED、约束烟测、MIG-009、U005 非空拒绝/空表卸载 PASS，库已删除；目标 `iot-device20` 未执行 V006，必须另行专项评审与窗口授权。U002～U005 仍未接入 uninstall 驱动，保持应用关闭 + 独立空表脚本边界。 |
| 1.5.4 | 2026-08-10 | V006 专项评审整改：新增对象身份与 assignment 历史不可变/版本关闭 trigger，最终 hash 更新为 `6fac9b42...9064`；第二临时库首次/重放/增强反例/MIG-009/U005 全 PASS并删除。形成 `V006落库窗口申请单-20260810.md`，状态待 owner 批准，目标未执行。 |
| 1.5.5 | 2026-08-10 | **V006 目标窗口已批准并执行**：approval=`USER-APPROVAL-20260810-V006`，runner 自动备份后仅执行 V006；目标 history/hash、五张空表、三函数六 trigger、MIG-009、invalid index 与业务计数验收通过，未启用 capability 或业务写入口。 |
| 1.5.6 | 2026-08-10 | V007 发布单派生身份通过专项评审并接入 runner apply 链：补同租户 binding/Outbox 复合 FK、事件幂等身份、稳定原因码与不可变保护；两轮临时库完整依赖链正反例 PASS 并删除。**仅完成 runner 增链，未对目标执行 V007**；目标 apply 仍须新批准和仓库外备份，U006 仍为独立空表卸载候选。 |
| 1.5.7 | 2026-08-10 | **V007 目标窗口已批准并执行**：approval=`USER-APPROVAL-20260810-V007`，runner 自动仓库外备份后仅执行 V007；history/hash、六列六约束、不可变函数、MIG-009、invalid index、目标 PG 合同及业务计数验收通过。第四端口配置仍关闭，未扩大授权到业务启用。 |
...
候选 runner 已存在。本 ADR 已于 1.5.0 转 **Accepted**（三项人工闭环全数关闭：DBA/代码 owner 双签、压测 owner 豁免、画像生产重跑 PASS 且 owner 指定满足 productionRerunRequired）。后续工作：事件消费者/transport/Inbox 实现与 CI 双主版本门禁接线按 TD-005 冻结计划推进；生产执行迁移仍须走当时窗口审批，候选预算值按当时环境人工评估。
