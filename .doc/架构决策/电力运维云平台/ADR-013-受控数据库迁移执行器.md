...
剩余 OPEN 收敛为：完整 12 表画像生产重跑、runner/CONCURRENTLY 在 standard 最低规格的性能压测、DBA/代码 owner 复核签字。
...
候选 runner 已存在；剩余 OPEN：完整 12 表画像/tenant 异常生产重跑、standard 最低规格性能压测、DBA/代码 owner 复核签字。`power_idempotency_record` 经 runner 正式落库的步骤建模已于 1.4.2 关闭（步骤 `M05`）。本 ADR 转 Accepted 前不得对生产库执行 DDL。
