...
> 状态：Proposed（已完成宪法专项设计处置、DBA/架构专项处置、Spike 与演练证据；转 Accepted 前仍须关闭完整画像生产重跑/压测/DBA 复核签字）
> 版本：1.4.1
> 日期：2026-08-07
...
td005-migration.sh apply [--db <database>] [--step M15|M16|V001|V002] [--approval <id>] [--yes]
...
| `--step` | 只执行指定步骤（M15/M16/V001/V002）；缺省按依赖顺序执行全部未完成步骤 |
...
剩余 OPEN 收敛为：完整 12 表画像生产重跑、runner/CONCURRENTLY 在 standard 最低规格的性能压测、`power_idempotency_record` 经 runner 正式落库的步骤建模（当前为手工候选验证）、DBA/代码 owner 复核签字。
...
候选 runner 已存在；剩余 OPEN：完整 12 表画像/tenant 异常生产重跑、standard 最低规格性能压测、`power_idempotency_record` 经 runner 正式落库的步骤建模、DBA/代码 owner 复核签字。本 ADR 转 Accepted 前不得对生产库执行 DDL。
