# Edge / Media Processing — Python 层详细架构

> 基于整体架构文件 V9.17.0 + 代码结构深入分析
> Python 文件总数: 397+
...
Python 层是整个 EasyAIot 平台的 **AI 与网络核心**，负责视频分析、AI 推理训练、边缘计算运行时和节点管理，由 **4 大模块** 组成。
...
│                                                                         │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐ │
│  │  AI (:5000)  │  │ VIDEO (:6000)│  │ NODE (:9100) │  │    EDGE      │ │
│  │  模型训练推理  │  │  视频分析服务  │  │  节点代理     │  │  边缘运行时   │ │
│  │  Flask       │  │  Flask       │  │  Flask       │  │  CLI + MQTT  │ │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘ │
│         │                 │                 │                 │         │
│         └─────────────────┴─────────────────┴─────────────────┘         │
│                                    │                                     │
...
└── app/
    ...
    ├── services/                   ← 业务逻辑层 (核心)
    │   ├── alert_consumer_service.py   ← 告警 Kafka 消费
    │   ├── alert_hook_service.py       ← 告警 Webhook 回调
    ...
    │   ├── dvr_upload_service.py       ← DVR 录像 MinIO 上传
    ...
    │   ├── playback_disk_guard_service.py ← 录像磁盘空间守护
    │   ├── post_process_* (4 文件)     ← Kafka 后处理编排
    │   ├── pusher_service.py           ← 视频推流服务
    │   ├── record_video_service.py     ← 录像协调服务
    │   ├── snap_* (4 文件)             ← 快照抓拍/上传
...
| 子服务 | 职责 |
|--------|------|
| **realtime_algorithm_service** | 实时视频流 AI 分析：YOLO 检测 → ByteTrack 跟踪 → RTMP 推流 → 告警输出 |
...
**定位**：超轻量边缘计算运行时，~512MB 内存，MQTT 全控制面，零本地磁盘占用。
...
EDGE/
├── edge/
│   ├── __init__.py              ← 包定义, __version__ = '0.1.0'
│   ├── __main__.py              ← 入口: python -m edge → cli.main()
│   ├── cli.py                   ← CLI 命令行 (config/enroll/run/stop/task)
│   ├── config.py                ← 本地配置 (edge.env + edge.state.json)
│   ├── node_client.py           ← HTTP 客户端 → iot-node 注册/配置
│   ├── mqtt_runtime.py          ← MQTT 运行时 (订阅/有序重连)
│   ├── workload_runner.py       ← 子进程管理 (Popen/进程树/PID持久化)
│   └── task_publish.py          ← MQTT 任务下发
├── runtime/
│   ├── services/
│   │   ├── realtime_algorithm_service/
│   │   │   └── run_deploy.py    ← 实时分析 (~4676 行, YOLO+跟踪+推流+告警)
│   │   ├── snapshot_algorithm_service/
│   │   │   └── run_deploy.py    ← 快照分析 (Cron 定时 + 推理)
│   │   └── patrol_algorithm_service/
│   │       └── run_deploy.py    ← 巡检服务 (轮巡/轮转模式)
│   ├── lib/
│   │   └── algo_mqtt_bus.py     ← 组件间 MQTT 通信总线
│   └── overlays/
│       └── apply_overlays.py    ← 叠加层可视化
└── requirements.txt
```
...
│                    EDGE 边缘运行时                             │
│                                                             │
│  CLI 入口: python -m edge <command>                          │
│                                                             │
│  ┌──────────┐  ┌────────────┐  ┌────────────────────────┐   │
│  │ config   │  │ enroll     │  │ run                     │   │
│  │ 配置中心  │  │ 节点注册    │  │ 主运行时 (MQTT 订阅)    │   │
│  │ edge.env │  │ → iot-node │  │ → 接收算法任务指令       │   │
│  └──────────┘  └────────────┘  └───────────┬────────────┘   │
│                                            │                │
│                          ┌─────────────────┼───────────┐    │
│                          │  workload_runner             │    │
...
│               │                        │              │     │
│  ┌────────────▼─────┐  ┌──────────────▼──┐  ┌───────▼──┐  │
│  │ realtime_service │  │ snapshot_service│  │ patrol   │  │
│  │ 实时分析子进程    │  │ 快照分析子进程   │  │ 巡检子进程│  │
│  │ YOLO + 推流+告警 │  │ 定时抓拍+推理   │  │ 轮巡模式 │  │
│  └──────────────────┘  └─────────────────┘  └──────────┘  │
│                                                             │
│  关键原则:                                                   │
│  • 不落本地业务盘 / 不直传 MinIO                             │
...
| 模块 | 文件 | 职责 |
|------|------|------|
| **CLI** | `cli.py` | 命令行入口: config/enroll/run/stop/task |
| **配置管理** | `config.py` | 读写 `edge.env` + `edge.state.json` |
| **节点客户端** | `node_client.py` | HTTP 向 iot-node 注册, 获取运行时配置 |
| **MQTT 运行时** | `mqtt_runtime.py` | 订阅 `mqtt/iot-algo-task-cmd`, 有序 Broker 重连 |
| **工作负载运行器** | `workload_runner.py` | subprocess.Popen 启动/终止算法子进程, PID 持久化 |
| **任务发布** | `task_publish.py` | MQTT 本地/远程任务下发 |

### 4.5 运行时子服务

| 子服务 | 文件 | 行数 | 功能 |
|--------|------|------|------|
| **realtime_service** | `runtime/services/realtime/run_deploy.py` | ~4676 | YOLO 检测 + ByteTrack 跟踪 + RTMP 推流 + 三重告警 |
| **snapshot_service** | `runtime/services/snapshot/run_deploy.py` | — | Cron 定时抓拍 + 单帧推理 + 告警 + MinIO 上传 |
| **patrol_service** | `runtime/services/patrol/run_deploy.py` | — | 轮巡/轮转模式, 多路摄像头循环分析 |
...
| 模块 | 路径 | 职责 |
|------|------|------|
| **cluster_storage** | `.scripts/lib/cluster_storage/__init__.py` | 集群模式存储管理: CephFS 挂载, 目录解析 (playbacks/snaps/datasets/models) |
| **model_resolver** | `.scripts/lib/model_resolver/__init__.py` | 模型路径解析: 本地存储 / Cluster (CephFS) / MinIO 三种模式统一解析 |
...
| **AI 训练** | iot-dataset 标注 → AI (:5000) train → export → deploy → VIDEO/TASK 推理 |
| **边缘推理** | 云中心 (EMQX) → MQTT 指令 → EDGE CLI → 子进程 YOLO → Ceph 汇聚 |
| **指令下发** | WEB → iot-device → Kafka → iot-sink (Modbus/OPC UA) / NODE Agent → EDGE |
...
| **MQTT 命令总线** | EDGE | 云边控制面: 任务指令通过 EMQX MQTT 下发 |
| **Kafka 事件总线** | VIDEO | 告警/人脸/车牌匹配结果异步流转 |
| **子进程工作负载** | EDGE / NODE / AI | subprocess.Popen 管理算法进程生命周期 |
| **Docker Compose 堆栈** | NODE Agent | SRS/ZLM/EMQX 的 Docker 容器生命周期管理 |
| **GPU 多卡调度** | AI / VIDEO / EDGE | 哈希/轮询策略分配 GPU 设备给不同流 |
...
| **Host 原生进程** | EDGE CLI | 需要启动子进程 + Ceph 挂载 |
| **Host 原生进程** | TASK (C++) | 编译后的原生二进制 |

> **原因**: NODE Agent 和 EDGE 必须在宿主机运行, 因为它们需要调用 Docker CLI 管理容器、挂载 CephFS、管理子进程生命周期。AI 和 VIDEO 则容器化以便于 CUDA/FFmpeg 环境管理和版本控制。
...
| EDGE | paho-mqtt, requests, psutil (精简) |
...
| EDGE CLI | — | MQTT Client (无服务端口) |
| ai_service (部署子服务) | 自动分配 | HTTP (Flask) |
| llm_service | 自动分配 | HTTP (Flask) |

---

> **一句话总结:** Python 层是 EasyAIot 的 **AI 智能化引擎** — AI 负责模型全生命周期 (训练→导出→部署→推理), VIDEO 负责实时视频分析 (检测→跟踪→告警→推流), NODE Agent 负责边缘控制面管理 (工作负载+媒体+MQTT 堆栈), EDGE 负责超轻量边缘计算运行时 (~512MB/MQTT 全控制面)。四模块通过 Nacos + MQTT + Kafka 协作, 形成从云端训练到边缘推理的完整 AIoT 闭环。
