...
> 日期：2026-07-30  
> 上游基线：[PRD 文档集 1.3.0](../../产品需求/电力运维云平台/README.md)、[平台功能计划 1.4.0](../../架构设计/平台功能计划.md)、[项目开发宪法 1.5.0](../../开发规范/EasyAIoT项目开发宪法.md)
...
|---|---|---|---|
| SPEC-005 | 复合告警规则与状态机 | 四级告警；`IGNORED` 临时抑制且持续评估；`FALSE_ALARM` 不可逆；统一 `alarmId` | 有界 AND/OR 白名单运算符；条件数、嵌套、时间窗、执行预算；重复事件幂等；迁移/回滚 |
| SPEC-006 | 值班通知与告警升级 | 通知失败不阻止告警；请求持久化；只在有效窗口补发；过期转备用渠道/升级 | 值班表冲突、渠道适配、退避/死信、送达口径；平台提交成功率 MVP ≥99% 的分母和排除项 |
| SPEC-007 | 安全遥控与操作票 | `iot-device` 唯一责任；双人复核、联锁、审计不可降级；默认 5 分钟、最大 15 分钟、单次执行 | 请求/审批/执行状态机；命令幂等；过期和条件变化失效；回执/超时；standard 依赖不足时整体关闭 |
