...
# TD-005 tenant 123 Canary identity/data read-only preflight wrapper for Windows PowerShell.
...
Invoke-ReadOnlySql -ScriptPath (Join-Path $directory 'preflight_canary_identity.sql') `
    -Database $SystemDatabase
Invoke-ReadOnlySql -ScriptPath (Join-Path $directory 'preflight_canary_tenant_data.sql') `
    -Database $DeviceDatabase
'CANARY_TENANT123_READINESS=PASS allowedLinks=3 forbiddenLinks=0 tenantResidualRows=0 encoding=utf8' |
    Write-Output
